...
# ── Data + model ───────────────────────────────────────────────────────
REQUIRED_COLS = {"Country Code", "Aggregate rating", "Votes", "Price range",
...
def load_uploaded_data(file_bytes: bytes):
    """Reads a user-uploaded Zomato-style CSV. Tries a couple of common encodings."""
    ...
    for enc in ("latin-1", "utf-8", "utf-8-sig"):
        ...
        raise ValueError("Could not read the file with a supported encoding.")
...
df = None  # resolved after the sidebar uploader below
...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Upload your own CSV (optional)", type=["csv"],
                                 help="Needs the same columns as the Zomato dataset: "
                                      "Country Code, Aggregate rating, Votes, Price range, "
                                      "Average Cost for two, Has Table booking, Has Online delivery.")
    if uploaded is not None:
        try:
            df = load_uploaded_data(uploaded.getvalue())
            st.success(f"Using your file · {df.shape[0]:,} rows")
        except Exception as e:
            st.error(f"Couldn't use that file: {e}")
            st.caption("Falling back to the sample dataset.")
