...
.chili-card {{ border-left: 3px solid {CHILI} !important; }}
.cardamom-card {{ border-left: 3px solid {CARDAMOM} !important; }}
.clove-card {{ border-left: 3px solid {CLOVE} !important; }}
div[data-testid="stMetricValue"] {{ color: {PAPER}; }}
.stButton>button {{
...
hr {{ border-color: #3A2C21; }}
.dataframe {{ background-color: {CHARCOAL_2}; }}
.footer-note {{
    color: {MUTED};
    font-size: 0.8rem;
    margin-top: 3rem;
...
# NOTE: fixed — this repo keeps zomato.csv / Country-Code.xlsx directly next
# to app.py (no "data/" subfolder), so we point straight at the app's own dir.
...
@st.cache_data
def load_data():
    csv_path = os.path.join(BASE_DIR, "zomato.csv")
    xlsx_path = os.path.join(BASE_DIR, "Country-Code.xlsx")

    missing = [p for p in [csv_path, xlsx_path] if not os.path.exists(p)]
    if missing:
        st.error(
            "**Data files not found.** This app expects `zomato.csv` and "
            "`Country-Code.xlsx` to sit in the **same folder** as `app.py`.\n\n"
            f"Missing: {', '.join(os.path.basename(p) for p in missing)}\n\n"
            "If you're on **Streamlit Cloud**: make sure both files are pushed to your "
            "GitHub repo at the top level, next to `app.py` (check `.gitignore` — CSVs "
            "sometimes get excluded by accident):\n\n"
            "```\n"
            "your-repo/\n"
            "├── app.py\n"
            "├── requirements.txt\n"
...
# ============================================================================
    ...
    valid = fdf[(fdf["Aggregate rating"] > 0) & (fdf["Votes"] > 0)]
    plot_df = valid.sample(min(1500, len(valid)), random_state=1) if len(valid) > 0 else valid
    if len(plot_df) > 0:
        fig3 = px.scatter(
